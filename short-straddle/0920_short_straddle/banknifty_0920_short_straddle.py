#  BANK NIFTY 0920 Short straddle, % based SL
...
def get_banknifty_atm_strike(ltp):
    r = ltp % 100  # Bank NIFTY strikes are in multiples of 100
    if r < 50:
        atm = ltp - r
    else:
        atm = ltp - r + 100
...
def get_banknifty_ltp():
    ...
    while a < 10:
        try:
            bn = kite.ltp('NSE:BANKNIFTY')
            bn_ltp = bn['NSE:BANKNIFTY']['last_price']
            break
        except:
...
def calculate_atm_and_place_order():
    ...
    ce_order_id = marketorder_sell(ce_symbol, lots * 15)  # Bank NIFTY lot size is 15

    pe_order_id = marketorder_sell(pe_symbol, lots * 15)  # Bank NIFTY lot size is 15
    ...
    ce_sl_orderid = stoploss_order_buy(ce_symbol, lots * 15, float(round_5ps(ce_sell_price + ce_stoploss_value)))
    pe_sl_orderid = stoploss_order_buy(pe_symbol, lots * 15, float(round_5ps(pe_sell_price + pe_stoploss_value)))
...
if ce_sl_status == 'pending':
    ...
    marketorder_buy(ce_symbol, lots * 15)  # Bank NIFTY lot size is 15

if pe_sl_status == 'pending':
    cancel_order(pe_sl_orderid)
    marketorder_buy(pe_symbol, lots * 15)  # Bank NIFTY lot size is 15