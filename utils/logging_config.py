...
def setup_logger(
    ...
    if console:
        ...
        console_format = TradingFormatter(
            fmt="%(asctime)s - %(name)s - %(levelname)s - %(message)s", datefmt="%Y-%m-%d %H:%M:%S", use_colors=True
        )
...
def setup_trade_logger(strategy_name: str, log_level: str = "INFO") -> logging.Logger:
    """
    ...
    """
    logger = setup_logger(
        strategy_name, level=log_level, log_dir="logs/strategies", console=True, file_logging=True, json_logging=False
    )
    ...
    trade_handler = logging.handlers.RotatingFileHandler(
        trade_log_path / f"{strategy_name}_trades.log", maxBytes=10 * 1024 * 1024, backupCount=10
    )
    trade_handler.setLevel(logging.INFO)
    trade_formatter = TradingFormatter(
        fmt="%(asctime)s - %(levelname)s - %(message)s", datefmt="%Y-%m-%d %H:%M:%S", use_colors=False
    )
    ...
    error_handler = logging.handlers.RotatingFileHandler(
        error_log_path / f"{strategy_name}_errors.log", maxBytes=5 * 1024 * 1024, backupCount=5
    )
